...
class McListBox():
    ...
    def __init__(self,table_header = None,table_list = None):
        ...
        self.tree = None
        #self.detailWin = tk.Tk()
        #self.detailWin.wm_title("multicolumn ListBox")
        #self.detailWin.geometry("305x300")
        self._setup_widgets()
        self._build_tree()
...
def scanqr():
    # inputVal = ''
    # inputVal = raw_input("plese input code:")
    # print 'get str:',inputVal
    # the test data ...
    car_header = ['name', 'value']
    car_list = [
        ('Hyundai', 'brakes') ,
        ('brakes', 'inputVal') ,
        ('Lexus', 'batteriiiiiiiiiiiiiigggiy') ,
        ('BMW', 'seat')
    ]
...
if __name__ == "__main__" :
    ...
    mainWin=tk.Tk()
    mainWin.title('System of QrCode')
    
    frame = Frame(mainWin,width = 100,height = 20)
    frame.bind('<Key>',printkey)
    frame.pack()
    button1 = tk.Button(mainWin,text="scanQr", command = scanqr)
    button1.pack()
